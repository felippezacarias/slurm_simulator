...
/* Local definition of this structure.  Should be a rough equivalent of what is in .../src/common/job_resources.h */

typedef struct job_resources {
...
main(int argc, char* argv[]) {
	...
	if((trace_file = open("test.trace", O_CREAT | O_RDWR, S_IRUSR |
...
recordJob(slurm_job_info_t* j) {
	 ...
	 * this tool is to create the job simulation input file based upon
	 ...
	 *	   real system at snapshot time
	 *	3) Expected time (the time limit or wclimit) if not finished
	 *	   (note finished jobs don't typically linger long in the queue)
	 * 	4) something else
	 * For simplicity, for now, we will assign the expected run time
	 * (time_limit/wclimit) value.  The user may always modify this trace
	 * file with the edit_trace program if this is not what is desired.
	 */
