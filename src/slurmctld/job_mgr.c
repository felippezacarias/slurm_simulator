...
//int _single_check_job_status(struct job_record *job_ptr, bool completing);
			...
			uint16_t protocol_version)
{
	...
	*job_pptr = job_ptr;
	...
	debug5("SDDEBUG: %s after _select_nodes_parts erro_code %d",__func__,error_code);
	if (!test_only) {
		last_job_update = now;
	}
			 ...
			 bool node_fail, uint32_t job_return_code)
{
	...
	debug5("SDDEBUG: %s node_env_state after deallocate job_id %u idle_nodes %u share_nodes %u avail_nodes %u",
...
extern int update_resize_sim_job(slurm_msg_t *msg, uid_t uid)
{
	...
	if (job_specs->pn_min_memory != NO_VAL64) {
		debug5("%s values orig_mem %u new_mem %u!",
				__func__,detail_ptr->pn_min_memory,job_specs->pn_min_memory);
				...
				uint32_t return_code)
{
	...
	debug5("SDDEBUG: %s job_id %u name %s job_nodes %u",__func__,job_ptr->job_id,node_name,job_ptr->node_cnt);
	...
	if(job_ptr->nodes_completing)
		debug5("SDDEBUG: %s job_id %u freeing nodes_completing %s",__func__,job_ptr->job_id,job_ptr->nodes_completing);
	xfree(job_ptr->nodes_completing);
...
extern void build_cg_bitmap(struct job_record *job_ptr)
{
	...
	if (job_ptr->node_bitmap) {
		...
		/* FVZ: also add memory pool bitmap to completing bitmap */
...
static int _update_sim_job_status(struct job_record *job_ptr){
	...
	if (slurm_send_recv_node_msg(&req_msg, &resp_msg, 500000) < 0) {
		...
		debug5("SDDEBUG: %s. job_id=%u sending update rpc success.", __func__,job_ptr->job_id);
	}
...
double _compute_scale(struct job_record *job_ptr){
	...
	if(job_cnt){
		...
		if(is_multi_curve){
			...
			info("SDDEBUG: %s. job_id=%u model_res_local=%.5f model_res_remote=%.5f local_remote_ratio=%.5f remote_allocation_ratio=%.5f",
			...
			info("SDDEBUG: %s multi_curve[%d] job_id=%u scale=%.5f bw_max=%.5f interf_nodes=%.1f bw_threshold=%.5f time_elapsed=%e",
			...
			while ((jobid = (uint32_t) list_next(job_iterator))) {
				...
				info("SDDEBUG: %s single_curve[%d] job_id=%u job_tmp=%u interf_exec=%u calc_interf_nodes=%u scale=%.5f bw_max=%.5f intef_nodes=%.1f time_elapsed=%e",
		...
		info("SDDEBUG: %s else job_id=%u scale=%.5f bw_max=%.5f interf_nodes=%.1f bw_threshold=%.5f time_elapsed=%e",
