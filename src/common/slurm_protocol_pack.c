...
pack_msg(slurm_msg_t const *msg, Buf buffer)
{
	...
	case REQUEST_UPDATE_SIM_JOB:
		//FELIPPE: pack function for update simjob info rpc. It uses the same of request_sim
		_pack_sim_job_msg((sim_job_msg_t *)msg->data, buffer);
		break;
...
unpack_msg(slurm_msg_t * msg, Buf buffer)
{
	...
	case REQUEST_SIM_JOB:
        ...
        case MESSAGE_SIM_HELPER_CYCLE:
                ...
                break;
	case REQUEST_UPDATE_SIM_JOB:
		//FELIPPE: unpack function for update simjob info rpc. It uses the same of request_sim
		_unpack_sim_job_msg((sim_job_msg_t **)&msg->data, buffer);
		break;
	case REQUEST_ABORT_JOB:
	case REQUEST_KILL_PREEMPTED:
