			       ...
			       void *node_rec_table, uint16_t fast_schedule)
{
	...
	bitmap_len = bit_set_count(job_resrcs->node_bitmap);
	job_resrcs->memory_nodes_index = xmalloc(bitmap_len * sizeof(int *));
	for(i = 0; i < bitmap_len; i++){
		job_resrcs->memory_nodes_index[i] = xmalloc(1 * sizeof(int *));
		//the first index mean how many remote mem nodes it has
		job_resrcs->memory_nodes_index[i][0] = 0;
	}
	...
	if (job_resrcs_ptr) {
		...
		if(job_resrcs_ptr->node_bitmap != NULL){
			for(int i = 0; i < bit_set_count(job_resrcs_ptr->node_bitmap); i++)
				xfree(job_resrcs_ptr->memory_nodes_index[i]);
		}
		xfree(job_resrcs_ptr->memory_nodes_index);
