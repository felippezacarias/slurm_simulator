...
static int simulator_update_event_info(sim_job_msg_t *req);
...
slurmd_req(slurm_msg_t *msg)
{
	...
	case REQUEST_UPDATE_SIM_JOB:
		...
		info("SDDEBUG: Processing RPC: REQUEST_UPDATE_SIM_JOB");
...
int simulator_add_future_event(batch_job_launch_msg_t *req){
	...
	if(!temp_ptr){
		info("SIM: No jobid event matching this jobid %u", req->job_id);
		pthread_mutex_unlock(&simulator_mutex);
		return -1;
	}
	...
	//If necessary it is already implemented
...
simulator_update_event_info(sim_job_msg_t *req)
{
	...
	if(!temp_ptr){
			info("SDDEBUG: %s jobid=%u  something went wrong!!",__func__,req->job_id);
			return SLURM_ERROR;
	}

	info("SDDEBUG: %s jobid=%u prev duration %u  now %u",
						__func__,req->job_id, temp_ptr->duration, req->duration);
...
simulator_rpc_update_job(slurm_msg_t *msg)
{
	...
	if(!head_simulator_event){
			info("SDDEBUG: %s There is no events! We can't update jobid=%u in head_simulator_event!",__func__,req->job_id);
			...
			info("SDDEBUG: %s Updating simulation time for jobid=%u in the event list to duration=%u",__func__,req->job_id, req->duration);
			...
			if(req->job_id == node_temp->job_id){
				info("SDDEBUG: %s jobid=%u before %ld now at %ld . head_simul at %ld head_id %u",
			...
			if((node_temp->next) && (node_temp->when > node_temp->next->when)){
				...
				info("SDDEBUG: %s if - jobid=%u prev at %ld  job_at %ld",
				...
				if(head_simulator_aux) head_simulator_event = head_simulator_aux;
					...
					if((next) && (next->job_id == head_simulator_event->job_id)){
						...
						info("SDDEBUG: %s else-if jobid=%u head_simulator_event at %ld job_at %ld head_id %u",
								__func__,req->job_id, head_simulator_event->when,
								node_temp->when,head_simulator_event->job_id);

					}else{
						info("SDDEBUG: %s else-else jobid=%u prev at %ld  job_at %ld",
								__func__, req->job_id, prev->when, node_temp->when);
...
simulator_rpc_terminate_job(slurm_msg_t *rec_msg)
{
	...
	debug5("SDDEBUG: %s jobid %u memory_nodes %s ",__func__,req_kill->job_id,req_kill->memory_nodes);
