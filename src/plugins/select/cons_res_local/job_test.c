/*****************************************************************************\
 *  select_cons_res_local.c - node selection plugin supporting consumable
 *  resources policies.
 *****************************************************************************\
 *
...
#include "job_test.h"
#include "select_cons_res_local.h"
...
static uint16_t *_select_nodes(struct job_record *job_ptr, uint32_t min_nodes,
				...
				bitstr_t *node_map,  bitstr_t *memory_pool_map, uint32_t cr_node_cnt,
				bitstr_t *core_map,
			      ...
			      bool will_run, bitstr_t *part_core_map)
{
	...
	/* FVZ: this step returns the core bitmap os available cores */
	if (cr_type & CR_CORE) {
	...
	/*FVZ: if the node has core available we use. Memory check is done before */
			      ...
			      bitstr_t *exc_core_bitmap, bool qos_preemptor)
{
	...
	/ * later we can improve it as the node can have free cores
	/ *: that can be used */
	/*if (job_ptr->details->pn_min_memory & MEM_PER_CPU) {
		...
		if (min_cpus > 0)
			min_mem *= min_cpus;
		debug5("SDDEBUG: %s job_id %u min_cpus %u", __func__,job_ptr->job_id,min_cpus);
	} else {
		min_mem = job_ptr->details->pn_min_memory;
	}*/
	//debug5("SDDEBUG: %s job_id %u min_mem %lu", __func__,job_ptr->job_id,min_mem);
	...
	for (i = i_first; i <= i_last; i++) {
		...
		/*FVZ: we check memory on _select_nodes */
		...
		//	else
		//		free_mem = 0;
		//	/* FVZ: Instead of comparing free and min, to support min_mem higher than node memory,
		//	 * we only clear bit if free_mem == 0; otherwise we can use this node */
		//	if (!free_mem) {
		...
		//			free_mem, min_mem);
		//		goto clear_bit;
		//	}else if((node_usage[i].alloc_memory !=  0) && (job_ptr->details->share_res == 0)){
		//		debug5("SDDEBUG: %s job_id %u requested exclusive node, node %s mem_usage %lu!",
		//		__func__,job_ptr->job_id,select_node_record[i].node_ptr->name,node_usage[i].alloc_memory);
		//		goto clear_bit;
		//	} 
...
/* Compute memory resource usage for all available resources
 *
			   ...
			   struct node_use_record *node_usage, bool test_only)
{
	...
	for (int node_i = 0; node_i < cr_node_cnt; node_i++) {
		...
		if(avail_mem == 0){
			bit_clear(memory_pool_map, node_i);
			bit_clear(local_mem_map,node_i);
		}
	}
}
...
/*_avail_memory - Given the bitmap of allocate nodes regarding core count,
* tasks, tasks_per_node and so on, confirm if the set of nodes has enough 
* memory to execute the job. Otherwise, it will iterate over nodes with 
* free memory and add them to the job node list. It is a approximation.
* The real allocation is done at the end of cr_job_test
* IN job_ptr - job pointer to access job_details
* IN node_map - bitmap of select nodes
			...
			struct node_use_record *node_usage, bool test_only)
{
	...
	int i_first, i_last, mem_nodes = 0;
	char *node_name,*mem_node_name;
	bitstr_t *orig_map;

	//debug5("SDDEBUG: %s for job_id %u",__func__,job_ptr->job_id);
	...
	if (job_ptr->details->pn_min_memory & MEM_PER_CPU) {
		...
		* I'm using the number of nodes in this case */
		//debug5("SDDEBUG: %s for job_id %u tasks %u tasks_per_node %u min_cpu %u cpu_per_task %u nodes %u",
		...
		if(job_ptr->details->num_tasks > 1){
			 ...
			 * num_tasks/ntasks_per_node is odd */
			if(job_ptr->details->ntasks_per_node) {
				min_cpus = nodes;
		...
		debug5("SDDEBUG: %s for job_id %u percpu %lu tot_req_mem %lu using min_cpus %u node_name %s",
				__func__,job_ptr->job_id,req_mem,tot_req_mem,min_cpus,node_name);
	}
	else{
		/* memory is per node */
		tot_req_mem = nodes * req_mem;
		debug5("SDDEBUG: %s for job_id %u per-node %lu tot_req_mem %lu using %d nodes %s",
	...
	/* FVZ: Iterate over chosen nodes and accumulate its total available mem */
	for (i = i_first; i <= i_last; i++) {
		if (!bit_test(node_map, i))
			continue;
		free_mem = select_node_record[i].real_memory;
	...
	if(tot_alloc_mem < tot_req_mem){
		...
		for (i = i_first; i <= i_last; i++) {
			...
			free_mem = select_node_record[i].real_memory;
			if(!test_only)	free_mem -= node_usage[i].alloc_memory;
			/* FVZ: Double checking the memory available */
			if(free_mem == 0){
				bit_clear(orig_map,i);
				continue;
			}		
			tot_alloc_mem += free_mem;
			bit_set(memory_pool_map,i);
			debug5("SDDEBUG: %s [memory node %s free_mem %lu] total allocated up now %lu",
					__func__,select_node_record[i].node_ptr->name,free_mem,tot_alloc_mem);
			mem_nodes++;
			if(tot_alloc_mem >= tot_req_mem){
				error_code = SLURM_SUCCESS;
				mem_node_name = bitmap2node_name(memory_pool_map);
				debug5("SDDEBUG: %s for job_id %u requested memory achieved using %d memory nodes %s",
	...
	if(tot_alloc_mem < tot_req_mem) {
		debug5("SDDEBUG: %s for job_id %u was not possible to allocate enough memory %lu for this job. Required %lu!!",__func__,job_ptr->job_id,tot_alloc_mem,tot_req_mem);		
		return SLURM_ERROR;
	}
	return error_code;
}
			 ...
			 bool prefer_alloc_nodes)
{
	...
	/* FVZ: Here we can't clear nodes without checking if they have 
	 * free memory */
	for (i = 0; i < cr_node_cnt; i++) {
		if (!bit_test(node_map, i))
			continue;
				...
				bool prefer_alloc_nodes)
{
	...
	struct job_details *details_ptr = job_ptr->details;
	bitstr_t *req_map = details_ptr->req_node_bitmap;
	bitstr_t *orig_map, *local_mem_map;
	...
	/*FVZ: _get_mem_usage() - create the memory_pool_bitmap here. to select nodes
	without available core, but with available memory. The next loop will remove those
	kind of nodes, but we need them  */
	local_mem_map = bit_copy(memory_pool_map);
	_get_mem_usage(memory_pool_map, local_mem_map, cr_node_cnt, node_usage, test_only);
	...
	/* FVZ: remove nodes without local memory to alaways allocate node with local memory.
	   If node size does not satisfy the requeriment we use core nodes without memory. */
	debug5("SDDEBUG: %s count before memory_pool_bitmap %u local_mem_map %u node_map %u",__func__,bit_set_count(memory_pool_map),bit_set_count(local_mem_map),bit_set_count(node_map));
	//orig_map = bit_copy(node_map);
	bit_and(node_map,local_mem_map);
	debug5("SDDEBUG: %s count after memory_pool_bitmap %u local_mem_map %u node_map %u",__func__,bit_set_count(memory_pool_map),bit_set_count(local_mem_map),bit_set_count(node_map));	
	FREE_NULL_BITMAP(local_mem_map);
	
	/* choose the best nodes for the job */
	rc = _choose_nodes(job_ptr, node_map, min_nodes, max_nodes, req_nodes,
			   cr_node_cnt, cpu_cnt, cr_type, prefer_alloc_nodes);
	debug5("SDDEBUG: %s After _choose_nodes 1 error_code %d",__func__,rc);
	_log_select_maps("_select_nodes/choose_nodes", node_map, core_map);

	/* Error due not enough cpu count */
	/*if(rc == SLURM_ERROR){
		bit_copybits(node_map,orig_map);
		rc = _choose_nodes(job_ptr, node_map, min_nodes, max_nodes, req_nodes,
				cr_node_cnt, cpu_cnt, cr_type, prefer_alloc_nodes);
		debug5("SDDEBUG: %s After _choose_nodes 2 error_code %d",__func__,rc);
		_log_select_maps("_select_nodes/choose_nodes", node_map, core_map);
	}*/
	//FREE_NULL_BITMAP(orig_map);

	/* FVZ: eval_nodes and _choose_nodes return the bitmap of nodes that satisfy user request
	* regarding cores and nodes, we have to garantee that total memory
	* is also satisfied. _eval_memory() returns nodes available to be memory pool.
	* bit_and_not return the difference between both bitmaps */	
	 ...
	 * create a cpus array */
	/* FVZ:  The core_map can't have the cores of the pool_mem nodes
	 * if cpu_cnt[i]=0 is sufficient */
	if (rc == SLURM_SUCCESS) {
			...
			bool qos_preemptor, bool preempt_mode)
{
	...
	if (!test_only) {
		/* FVZ: Here we clear nodes that has 0 free_mem */
	...
	//debug5("SDDEBUG: %s before _select_nodes 1 evaluating job %u on %u nodes test_only %d",
	...
	//debug5("SDDEBUG: %s after _select_nodes 1 job %u on %lu memory_nodes test_only %d!",
	...
	if (cpu_count == NULL) {
		...
		debug5("SDDEBUG: %s not enough resources job %u test_only %d!",
	...
	//debug5("SDDEBUG: %s before _select_nodes 2 evaluating job %u on %u nodes test_only %d",
	//		__func__,job_ptr->job_id, bit_set_count(node_bitmap),test_only);
	...
	if ((cpu_count) && (job_ptr->best_switch)) {
		/* job fits! We're done. */
		debug5("SDDEBUG: %s after _select_nodes 2 job %u on %u nodes and memory_nodes %u test_only %d job fits!",
			__func__,job_ptr->job_id, bit_set_count(node_bitmap), bit_set_count(memory_pool_map), test_only);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
	...
	if (cpu_count) {
		...
		for (p_ptr = cr_part_ptr; p_ptr; p_ptr = p_ptr->next) {
			...
			cpu_count_tmp = _select_nodes(job_ptr, min_nodes,
						max_nodes, req_nodes,
						node_bitmap_tmp, memory_pool_map, cr_node_cnt,
	...
	if (!jp_ptr || !jp_ptr->row) {
		...
		cpu_count = _select_nodes(job_ptr, min_nodes, max_nodes,
					  req_nodes, node_bitmap, memory_pool_map, cr_node_cnt,
...
alloc_job:
	...
	/* FVZ: here node_bitmap should have both compute and pool_mem nodes */
	...
	job_res->cpus_used        = xmalloc(job_res->nhosts *
					    sizeof(uint16_t));
	job_res->memory_allocated = xmalloc(job_res->memory_nhosts * sizeof(uint64_t));
	job_res->memory_used      = xmalloc(job_res->memory_nhosts * sizeof(uint64_t));
	...
	/* FVZ: make distinction, for compute nodes it alocates all the memory
	...
	uint64_t *mem_avail_size=NULL;
	uint32_t nodes;
	bool using_nodes = false;
	bool mem_avail = false;

	mem_avail_size = xmalloc(job_res->memory_nhosts * sizeof(uint64_t));
	memset(mem_avail_size,0,job_res->memory_nhosts);
	...
	if (save_mem & MEM_PER_CPU){/* memory is per-cpu */
		...
		if(job_ptr->details->num_tasks > 1){
			...
			min_cpus = MAX(job_ptr->details->min_cpus, nodes);
			info("SDDEBUG: %s Filling in job resources of job_id %u mem_per_cpu requested but using min_cpus %u or nodes %u to calculate memory requirement.",__func__,job_ptr->job_id,job_ptr->details->min_cpus,nodes);
		}
		...
		for (i = first; i <= last; i++) {
			...
			//if(rem > 0){
				...
				if(allocated >= avail){
					...
					rem -=  avail;
					mem_avail_size[idx_mem] = 0;
				}else{
					mem_avail = true;
					mem_avail_size[idx_mem] = avail-allocated;
					if(rem <= allocated){
			...
			//else
			//{
			//	job_res->memory_allocated[idx_mem] = 0;
			//}

			info("SDDEBUG: %s job_id %u node %s memory_allocated %lu node_usage %lu avail %lu rem %lu cpus %u",
		...
		info("SDDEBUG: %s Filling in job resources of job_id %u for memory_allocated %lu per node, total requested %lu",__func__,job_ptr->job_id,save_mem,rem);
		for (i = first; i <= last; i++) {
			if (!bit_test(job_res->memory_pool_bitmap, i))
				continue;
			...
			if((save_mem <= select_node_record[i].real_memory) && (save_mem <= avail)){
				...
				rem -= save_mem;
				mem_avail_size[idx_mem] = avail-save_mem;
			}
			else{
				job_res->memory_allocated[idx_mem] = avail;
				rem -= avail;
			...
			allocated = (node_usage[i].alloc_memory+job_res->memory_allocated[idx_mem]);
	...
	if(mem_avail){
		for(i=0;i<job_res->memory_nhosts;i++){
			if(((long)(rem - mem_avail_size[i])) <= 0){
					job_res->memory_allocated[i] += rem;
					rem = 0;
					break;
			}
			rem-=mem_avail_size[i];
			job_res->memory_allocated[i]+=mem_avail_size[i];
		}
	}
	xfree(mem_avail_size);
