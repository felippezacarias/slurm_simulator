...
static void _get_mem_usage(bitstr_t *memory_pool_map, uint32_t cr_node_cnt,
			   struct node_use_record *node_usage, bool test_only);
				...
				bool prefer_alloc_nodes)
{
	...
	_get_mem_usage(memory_pool_map, cr_node_cnt, node_usage, test_only);
	...
	debug5("SDDEBUG: %s count before memory_pool_bitmap %u node_map %u",__func__,bit_set_count(memory_pool_map),bit_set_count(node_map));
	orig_map = bit_copy(node_map);
	bit_and(node_map,memory_pool_map);
	debug5("SDDEBUG: %s count after memory_pool_bitmap %u node_map %u",__func__,bit_set_count(memory_pool_map),bit_set_count(node_map));	
