...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
    ...
    if ((tmp_ptr = xstrcasestr(sched_params, "alloc_group="))) {
		param_alloc_group = atoi(tmp_ptr + 12);
		if (param_alloc_group <= 0) {
			fatal("Invalid SchedulerParameters param_alloc_group: %d",
			      param_alloc_group);
		}
	} else
		param_alloc_group = 1;
