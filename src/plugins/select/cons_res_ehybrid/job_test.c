...
static uint16_t *_select_nodes(struct job_record *job_ptr, uint32_t min_nodes,
				...
				bool prefer_alloc_nodes, bool is_default);
			      ...
			      bool will_run, bitstr_t *part_core_map, uint64_t sched_weight, bool is_default)
{
	...
	if ((cr_type & CR_MEMORY) && is_default) {
		...
		node_ptr->sched_weight = (NO_VAL64 - 1) -
								avail_mem;
...
static void _get_res_usage(struct job_record *job_ptr, bitstr_t *node_map,
			   ...
			   bitstr_t *part_core_map, bool is_default)
{
	uint16_t *cpu_cnt;
	uint32_t n;
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		for (i = 0; i < consec_index; i++) {
			info("cons_res: eval_nodes:%d consec "
			     "c=%d n=%d b=%d e=%d r=%d weight=%u",
			     i, consec_cpus[i], consec_nodes[i],
			     consec_start[i], consec_end[i],
 ...
 * IN: is_default 	- to use the default approach or not when considering a node for scheduling
			...
			bool qos_preemptor, bool preempt_mode)
{
	...
	bool test_only, will_run, is_default;
	...
	//debug5("SDDEBUG: %s before _select_nodes [true] evaluating job %u on %u nodes test_only %d",
	...
	cpu_count = _select_nodes(job_ptr, min_nodes, max_nodes, req_nodes,
				  ...
				  part_core_map, prefer_alloc_nodes, is_default);

	/* FVZ: first we test the default approach, if we don't have enough resources
			we use the disaggregated approach */
	if(cpu_count == NULL){
		is_default = false;
		...
		//debug5("SDDEBUG: %s before _select_nodes [false] evaluating job %u on %u nodes test_only %d",
		//	__func__,job_ptr->job_id, bit_set_count(node_bitmap),test_only);
		cpu_count = _select_nodes(job_ptr, min_nodes, max_nodes, req_nodes,
					node_bitmap, memory_pool_map, cr_node_cnt, free_cores,
					node_usage, cr_type, test_only, will_run,
	...
	//debug5("SDDEBUG: %s after _select_nodes job %u on %lu memory_nodes test_only %d!",
...
alloc_job:
	...
	uint64_t avail = 0, rem = 0, rem_no_ov = 0, min_cpus, allocated, alloc_cpus = 0;
	...
	job_res->true_memory_pool_bitmap = bit_copy(job_res->node_bitmap);
	...
	if (save_mem & MEM_PER_CPU){/* memory is per-cpu */
		...
		for (i = first; i <= last; i++) {
			...
			if(alloc_cpus < job_ptr->ori_cpu){
				alloc_cpus+=job_res->cpus[idx_cpu];
	...
	if(rem){
		...
		ori_mem_node = bit_set_count(job_res->true_memory_pool_bitmap);
		...
		for (i = first; i <= last; i++) {
			...
			// Counting the memory nodes of original cpu nodes
			// since idx_cpu starts = 0, the comparison has to be <
			if(idx_cpu < ori_mem_node){
				bit_set(job_res->true_memory_pool_bitmap,i);
			}
