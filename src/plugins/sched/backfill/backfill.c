		       ...
		       uint32_t req_nodes, bitstr_t *exc_core_bitmap)
{
	...
	if (has_xand || feat_cnt) {
		//debug5("SDDEBUG: backfill: %s if (feat_cnt)  jobid %u",__func__,job_ptr->job_id);
		/*
		...
		//debug5("SDDEBUG: backfill: %s else if (has_xor)  jobid %u",__func__,job_ptr->job_id);
		/*
		 * Cache the feature information and test the individual
		 * features (or sets of features in parenthesis), one at a time
		 */
		...
		//debug5("SDDEBUG: backfill: %s else if (detail_ptr->feature_list)  jobid %u",__func__,job_ptr->job_id);
		if ((job_req_node_filter(job_ptr, *avail_bitmap, true) !=
		     SLURM_SUCCESS) ||
		    (bit_set_count(*avail_bitmap) < min_nodes)) {
			rc = ESLURM_NODES_BUSY;
		...
		//debug5("SDDEBUG: backfill: %s else jobid %u",__func__,job_ptr->job_id);
		...
		rc = select_g_job_test(job_ptr, *avail_bitmap, min_nodes,
				       ...
				       exc_core_bitmap);
		FREE_NULL_LIST(preemptee_job_list);
		//debug5("SDDEBUG: backfill: %s after select_g_job_test 1 jobid %u error_code %d",__func__,job_ptr->job_id,rc);
		//if(*avail_bitmap)
		//	debug5("SDDEBUG: backfill: %s after select_g_job_test 1 jobid %u avail_bitmap %d",__func__,job_ptr->job_id,bit_set_count(*avail_bitmap));
		...
		if (((rc != SLURM_SUCCESS) || (job_ptr->start_time > now)) &&
			...
			*avail_bitmap = tmp_bitmap;
			rc = select_g_job_test(job_ptr, *avail_bitmap,
					       ...
					       exc_core_bitmap);
			FREE_NULL_LIST(preemptee_job_list);
			//debug5("SDDEBUG: backfill: %s after select_g_job_test 2 jobid %u error_code %d",__func__,job_ptr->job_id,rc);
			//if(*avail_bitmap)
			//	debug5("SDDEBUG: backfill: %s after select_g_job_test 2 jobid %u avail_bitmap %d",__func__,job_ptr->job_id,bit_set_count(*avail_bitmap));
...
next_task:
		...
		//debug5("SDDEBUG: backfill: %s after get_node_cnts jobid %u min_nodes %u max_nodes %u req_nodes %u error_code %d",
 ...
 TRY_LATER:
		...
		if (test_fini != 1) {
			...
			//debug5("SDDEBUG: backfill: %s before _try_sched jobid %u avail_nodes %d",__func__,job_ptr->job_id,bit_set_count(avail_bitmap));
			j = _try_sched(job_ptr, &avail_bitmap, min_nodes,
				       max_nodes, req_nodes, exc_core_bitmap);
   			//debug5("SDDEBUG: backfill: %s after _try_sched jobid %u test_fini %d error_code %d",__func__,job_ptr->job_id,test_fini,j);
			if (test_fini == 0) {
		...
		if ((job_ptr->start_time <= now) &&
			...
			//debug5("SDDEBUG: backfill: %s after _start_job jobid %u error_code %d",__func__,job_ptr->job_id,rc);
			...
			if((error_code == SLURM_SUCCESS) &&
				...
				info("SDDEBUG: %s[%llu] Cancelling jobid %u time_limit %u ready to run. ret_code %d",
					 __func__,now,job_ptr->job_id,job_ptr->time_limit,error_code);
				continue;
			}
			#endif
...
static int _start_job(struct job_record *job_ptr, bitstr_t *resv_bitmap)
{
	...
	//debug5("SDDEBUG: backfill: %s jobid %u",__func__,job_ptr->job_id);
	...
	//debug5("SDDEBUG: backfill: %s after select_nodes jobid %u error_code %d",__func__,job_ptr->job_id,rc);
	/* FVZ: Dealing with cancelled jobs */
	#ifdef SLURM_SIMULATOR		
